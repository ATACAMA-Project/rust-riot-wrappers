...
impl <'a> EcdsaParams <'a> {
    pub fn new <const EC_CLIENT_KEYS_NUM: usize> (private_key: &'a[u8], public_key: EcdsaPublicKey<'a>, client_keys: &'a EcdsaClientKeys<EC_CLIENT_KEYS_NUM>) -> Self{
        EcdsaParams{
            private_key,
            public_key,
...
impl < const EC_CLIENT_KEYS_NUM: usize> EcdsaClientKeys< EC_CLIENT_KEYS_NUM> {
    fn new (client_keys: &[EcdsaPublicKey]) -> Self{
        let mut keys = EcdsaClientKeys{
            client_keys_c:[riot_sys::ecdsa_public_key_t{x: ptr::null(), y: ptr::null(),};EC_CLIENT_KEYS_NUM],
        };
        let mut i=0;
...
Main: FnOnce() -> RMain,
{
    ...
    let ret= main();
    unsafe{ riot_sys::credman_delete(credential.credential.tag,credential.credential.type_);}
    Ok(ret)
}
...
fn credman_add(credential: &Credential) -> CredmanStatus{
    let res = unsafe {riot_sys::credman_add(&credential.credential as *const riot_sys::credman_credential_t)};
    CredmanStatus::from_value(res)
}

// int credman_get(credman_credential_t *credential, credman_tag_t tag, credman_type_t type);
pub fn credman_get( tag: CredmanTag, typ: CredmanType) -> Result<CredentialRef,CredmanStatus>{ //TOD: CredmanCredRef zurückgeben
    let mut cred : riot_sys::credman_credential_t = Default::default();
    let res = unsafe{riot_sys::credman_get(&mut cred as *mut riot_sys::credman_credential_t,tag,CredmanType::to_c (typ))};
